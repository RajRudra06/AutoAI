...
class SchedulingAgent:
    ...
    def fetch_all_vehicles_globally(self) -> list:
        # This is temporary; ideally, there would be a filtered API endpoint for vehicles needing scheduling.
...
")
        ...
        try:
            ...
            post(
                ...
                json={
                    "vehicle_id": vehicle_id,
                    "pipeline_associated": {
                        "pipeline_status": "DIAGNOSIS_COMPLETE", # Rollback to stage before scheduling
                        "pipeline_assigned_at": datetime(1968, 1, 1, tzinfo=timezone.utc).isoformat()
                        # "celery_task_id": None # Clear task ID if rolling back
                    }
                }
            )
    ...
    def lifecycle_gate_check(self, vehicle: dict) -> bool:
        ...
        if isinstance(pipeline_assigned_at_str, str):
            try:
                # Corrected to use datetime.timezone.utc
        ...
        now = datetime.now(timezone.utc) # Corrected to use datetime.timezone.utc
        timeout = 60 # seconds

        # Expanded gate based on Master/Diagnosis Agent patterns
        # Block if already scheduled, no scheduling required, or assigned by this agent
        if (
            workflow_stage == "SCHEDULING_COMPLETE"
            or not scheduling_flag
            or pipeline_status == "ASSIGNED_BY_SCHEDULING_AGENT" # Block if already assigned by this agent
        ):
            ...
            if (
                ...
                and scheduling_flag # Still required, but stale
        ...
        # Only allow vehicles that have completed diagnosis and require scheduling
        if not (workflow_stage == "DIAGNOSIS_COMPLETE" and scheduling_flag):
            print(f"[SCHEDULING SHARD {self.shard_id}][GATE] Vehicle {vehicle_id} not ready for scheduling (Workflow: {workflow_stage}, Scheduling Flag: {scheduling_flag}).")
        ...
        try:
            update_req = post(
                ...
                json={
                    "vehicle_id": vehicle_id,
                    "pipeline_associated": {
                        "pipeline_status": "DIAGNOSIS_COMPLETE", # Reset to stage before scheduling
                        "pipeline_assigned_at": datetime(1968, 1, 1, tzinfo=timezone.utc).isoformat(), # Corrected timezone.utc
                        "celery_task_id": None
                    },
                    "workflow_state": {
                        "current_stage": "DIAGNOSIS_COMPLETE", # Ensure workflow state is consistent
                        "flags": {
                            "scheduling_required": True, # Ensure it still needs scheduling
...
if __name__ == "__main__":
    try:
        ...
        print("
[SCHEDULING ORCHESTRATOR] Shutdown signal received. Exiting.")
