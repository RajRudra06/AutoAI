...
from transition.service_completion_task import execute_service_completion_job # Import the new Celery task
...
class ServiceCompletionAgent:
    ...
    def fetch_all_vehicles_globally(self) -> list:
        # This is temporary; ideally, there would be a filtered API endpoint for vehicles needing service completion.
    ...
    def extract_vehicle_params(self, vehicle: dict) -> dict:
        ...
        return {
            ...
            "risk_state": risk_state, # Need to pass risk state to execution for final vehicle state update
            "temp_last_processed_telemetry": vehicle.get("temp_last_processed_telemetry"), # Needed for final vehicle state update
        }
...
")
    ...
    def enqueue_service_completion_task(self, vehicle: dict):
        ...
        # risk_state is not explicitly used by the task directly but passed to update_vehicle_state.
        # It's already in extract_vehicle_params, so it will be available in the task.
        ...
        try:
            ...
            update_vehicle_state_status = post(
                ...
                json={
                    "vehicle_id": vehicle_id,
                    "pipeline_associated": {
                        "pipeline_status": "ASSIGNED_BY_SERVICE_COMPLETION_AGENT", # New status
            ...
            post(
                ...
                json={
                    "vehicle_id": vehicle_id,
                    "pipeline_associated": {
                        "pipeline_status": "ENGAGEMENT_COMPLETE", # Rollback to stage before service completion
                    ...
                    "workflow_state": {
                        "current_stage": "ENGAGEMENT_COMPLETE", # Back to this stage if service completion failed
                        "flags": {
                            "diagnosis_required": False,
                            "scheduling_required": False,
                            "engagement_required": False # It still needs to be complete though
                        }
                    },
    ...
    def get_booking_status(self, vehicle_id: str) -> requests.Response:
        ...
        return booking_status_resp # Return response object, not boolean
    ...
    def lifecycle_gate_check(self, vehicle: dict) -> bool:
        ...
        if isinstance(pipeline_assigned_at_str, str):
            try:
                ...
                pass # Handle if string is not a valid isoformat

        now = datetime.now(datetime.timezone.utc)
        timeout = 60 # seconds

        # Check booking status first, it's essential for this agent
        ...
        if (
            workflow_stage == "IDLE" # Already completed and reset
            or workflow_stage != "ENGAGEMENT_COMPLETE" # Not yet at engagement complete stage
            or not booking_completed # Booking not completed yet
            or pipeline_status == "ASSIGNED_BY_SERVICE_COMPLETION_AGENT" # Block if already assigned by this agent
        ):
            # Check for stale tasks assigned by this agent
            if (
                pipeline_status == "ASSIGNED_BY_SERVICE_COMPLETION_AGENT"
                and pipeline_assigned_at
                and workflow_stage == "ENGAGEMENT_COMPLETE" # Stage before service completion
                and booking_completed # Booking should be completed for stale check
                and celery_task_id is not None
            ):
                if pipeline_assigned_at.tzinfo is None: # Ensure timezone awareness for comparison
        ...
        try:
            update_req = post(
                ...
                json={
                    "vehicle_id": vehicle_id,
                    "pipeline_associated": {
                        "pipeline_status": "ENGAGEMENT_COMPLETE", # Reset to stage before service completion
                        "pipeline_assigned_at": datetime(1968, 1, 1, tzinfo=datetime.timezone.utc).isoformat(),
                        "celery_task_id": None
                    },
                    "workflow_state": {
                        "current_stage": "ENGAGEMENT_COMPLETE", # Ensure workflow state is consistent
                        "flags": {
                            "diagnosis_required": False,
                            "scheduling_required": False,
                            "engagement_required": False # Should be false if engagement was completed
                        }
                    },
                    "risk_state": vehicle_state_params["risk_state"] # Preserve original risk state
                }
...
if __name__ == "__main__":
    try:
        ...
        print("
[SERVICE COMPLETION ORCHESTRATOR] Shutdown signal received. Exiting.")